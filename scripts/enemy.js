...
import {
  ...
  LANTERN_STATES, PLANE_STATES, FPS_DIV, HIT_FRAC
} from './constants.js';
...
export function maybeSpawn(frame, stopped) {
  if (stopped) return;                          // no waves after death
...
export function updateAndDraw(frame, stopped) {
  const speed = stopped ? 0 : BASE_SPEED;       // freeze x‑movement

  planes.forEach((p, i) => {
    if (frame % FPS_DIV === 0) {
      const max = PLANE_STATES[p.state];
      p.idx = (p.idx + 1) % max;
    }
    p.x -= speed;
  ...
  lanterns.forEach((l, i) => {
    if (frame % FPS_DIV === 0) {
      const max = LANTERN_STATES[l.state];
      l.idx = (l.idx + 1) % max;
    }
    l.x -= speed;
